...
import static net.fneifnox.mobtalisman.component.cca.MyComponents.GHAST_TALISMAN_ABILITY_READY;

public class GhastTalisman extends AccessoryItem {
    ...
    public GhastTalisman(Settings properties) {
        super(properties);
    }
    ...
    public static boolean useBoolean(Entity provider) {
        return GHAST_TALISMAN_ABILITY_READY.get(provider).getValue();
    }

    public static void setBooleanFalse(Entity provider) {
        BooleanComponent component = GHAST_TALISMAN_ABILITY_READY.get(provider);
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        String key = KeyInputHandler.ghastAbilityKey.getBoundKeyLocalizedText().getString();
        tooltip.add(Text.translatable("tooltip.mob-talisman.ghast_talisman.prefix")
        ...
        if (seconds > 0 && minutes <= 0) {
            tooltip.add(Text.translatable("tooltip.mob-talisman.ghast_talisman.cooldown.sec", seconds));
        }
        else if (minutes > 0 && secondsLeft == 0) {
            tooltip.add(Text.translatable("tooltip.mob-talisman.ghast_talisman.cooldown.min", minutes));
        }
        else if (minutes > 0 && secondsLeft > 0) {
            tooltip.add(Text.translatable("tooltip.mob-talisman.ghast_talisman.cooldown.min_sec", minutes, secondsLeft));
        }
        else if (seconds <= 0) {
            tooltip.add(Text.translatable("tooltip.mob-talisman.ghast_talisman.ready"));
        }

        if (CONFIG.showDropchancesAsTooltip()) {
            float dropchance = CONFIG.dropchanceForGhastTalisman();
